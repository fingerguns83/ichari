...
?>
<div class="w-full h-full py-6 bg-sky-300 rounded-2xl group">
        ...
        <div class="flex content-center items-center justify-center mt-2">
          @foreach ($coowners as $coowner)
            @if ($coowner->user_id !== Auth::id())
              ...
              <img class="rounded-full mx-2" src="{{$owner->discord_avatar}}" width="32" height="32" title="{{$owner->discord_name}}">
            @endif
          @endforeach
        </div>
      @endif
